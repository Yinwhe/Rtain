    ...
    fn handle(
        ...
        Box::pin(async move {
            match event {
                ...
                MetadataEvent::HealthChanged {
                    id,
                    old_health,
                    new_health,
                } => {
                    println!(
                        "❤️  Container {} health changed: {:?} -> {:?}",
...
pub async fn example_usage() -> anyhow::Result<()> {
    ...
    manager
        .update_container_resources(container_meta.id.clone(), resources)
        .await?;
    ...
    manager
        .attach_network(container_meta.id.clone(), network)
        .await?;
    ...
    manager
        .updates(container_meta.id.clone(), ContainerStatus::Running)
        .await?;
...
async fn demo_advanced_queries(manager: &ContainerManager) -> anyhow::Result<()> {
    ...
    println!(
        "  Running: {}/{}",
        summary.running_count, summary.total_count
    );
